# Ashlar — MUMPS AI Assistant (VS Code extension)

Ashlar is an AI coding assistant for MUMPS, the language behind most EHR
systems (Epic, VistA, Meditech). This extension embeds Ashlar's existing
Vite/React frontend in a VS Code sidebar panel so it can be used without
leaving the editor.

This extension is a thin shell: it does not reimplement Ashlar's UI. It
...
# Terminal 2 — Ashlar FastAPI backend, served at http://127.0.0.1:8000
uv run python -m ashlar.api.server

# Terminal 3 — Ashlar Vite/React frontend, served at http://localhost:5173
cd frontend
npm run dev
```
...
npm install
npm run compile
npm run package   # produces ashlar-mumps-assistant-<version>.vsix
```

Then install the generated `.vsix` file with either:

```bash
code --install-extension ashlar-mumps-assistant-<version>.vsix
```
...
## 3. Open the Ashlar sidebar

With the backend and frontend running and the extension installed, either:

- Click the **Ashlar** icon in the VS Code Activity Bar (left-hand sidebar), or
- Open the Command Palette (`Cmd+Shift+P` / `Ctrl+Shift+P`) and run
  **"Ashlar: Open Assistant"**.
...
**"Ashlar: Open in Browser"** from the Command Palette to open
